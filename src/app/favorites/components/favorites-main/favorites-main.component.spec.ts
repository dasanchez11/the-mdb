...
fdescribe('FavoritesMainComponent', () => {
  ...
  it('should render logged user information correctly', () => {
    ...
    expect(firstUsernameLetter.nativeElement.textContent.trim()).toEqual(userMock.username.slice(0,1).toUpperCase())
    const username = element.query(By.css(".username"))
    expect(username.nativeElement.textContent.trim()).toEqual(userMock.username)
  })

  it('should render the correct amount of favorite movies', () => {
    const favoritePreviews = element.queryAll(By.directive(MockFavoritesPreviewComponent))
    expect(favoritePreviews.length).toEqual(mockMovieList.length)
  })

  it('should pass the correct movie to favorite preview element', () => {
    const favoritePreviewComponentElement = element.query(By.directive(MockFavoritesPreviewComponent))
    favoritePreviewComponent = favoritePreviewComponentElement.injector.get(MockFavoritesPreviewComponent)
